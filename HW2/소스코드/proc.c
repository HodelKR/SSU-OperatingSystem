...
void init_proc_info(struct proc*);
...
// 프로세스 초기화 시 호출되는 함수
void init_proc_info(struct proc *p) {
  ...
  p->time_slice = time_quantum[p->q_level];
}
...
found:
  ...
  init_proc_info(p);  // 새로운 프로세스 생성 시 호출
  ...
  for(;;){
    ...
    for(int level = 0; level < NLEVEL; level++) {
      ...
      for(p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
        ...
        // 에이징 체크
        if(p->cpu_wait >= AGING_THRESHOLD && p->q_level > 0) {
          p->q_level--;
          p->cpu_wait = 0;
          p->cpu_burst = 0;
          p->io_wait_time = 0;
      ...
      if(selected) {
        ...
        // Time Quantum 소진 체크
        if(p->total_ticks >= p->time_slice && p->q_level < NLEVEL-1 && p->cpu_end > 0) {
          cprintf("PID: %d uses %d ticks in mlfq[%d], total(%d/%d)\n",
                p->pid, time_quantum[p->q_level], p->q_level,
                p->total_ticks, p->cpu_end);
          p->q_level++;
          p->cpu_burst = 0;
          p->cpu_wait = 0;
          p->io_wait_time = 0;
        ...
        // q_level이 3인 경우
        else if(p->total_ticks >= p->time_slice && p->cpu_end > 0 && p->q_level == NLEVEL-1){
          cprintf("PID: %d uses %d ticks in mlfq[%d], total(%d/%d)\n",
                p->pid, time_quantum[p->q_level], p->q_level,
                p->total_ticks, p->cpu_end);
          p->cpu_burst = 0;
...
void update_proc_times(void) {
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
    if(p->state == RUNNING){
      ...
      p->total_ticks++;
      // 실행시간을 모두 완료하면 종료
      if(p->cpu_end > 0 && p->total_ticks >= p->cpu_end) {
        cprintf("PID: %d uses %d ticks in mlfq[%d], total(%d/%d)\n",
                p->pid, time_quantum[p->q_level], p->q_level,
                p->total_ticks, p->cpu_end);
